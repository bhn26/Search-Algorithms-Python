...
#IDDFS takes in a list representation of the board and runs an
#iterative deepening DFS algorithm on it
def IDDFS(board):
	...
	#Initialize the max depth of the alg
	MD = 13

	#Starts the iterative depth of the IDDFS
	for currD in range(MD):

		#Initialize stack and push pair containing travel path and state
		q = LifoQueue()
		q.put(([], start, 0))
		...
		#Runs DLS while stack is not empty
		while not(q.empty()):
			
			#Gets the item on top of the queue
			(traverse, tmp_state, depth) = q.get()
			...
			#Otherwise expand the stack with the neighbors while still in
			#depth range
			elif depth < currD:
				...
				#state to the stack. It does this for the other three
				#directions too.
				if(R == 1):
					...
					if state_space(moved_state) not in visited:
						...
						q.put((list(new_traverse), list(moved_state), depth+1))
				if(L == 1):
					moved_state = deepcopy(new_state)
					moved_state = make_move(moved_state, zpos[0], zpos[1], zpos[0], zpos[1]-1)
